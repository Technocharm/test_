...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RIGHT and (k[in_x+1][in_y]==1):
                in_x+=1
                velocity_x = init_velocity 
                velocity_y = 0

            if event.key == pygame.K_LEFT and (k[in_x-1][in_y]==1):
                in_x-=1
                velocity_x = - init_velocity
                velocity_y = 0

            if event.key == pygame.K_UP and (k[in_x][in_y-1]==1):
                in_y-=1
                velocity_y = - init_velocity
                velocity_x = 0

            if event.key == pygame.K_DOWN and (k[in_x][in_y+1]==1):
                in_y+=1
                velocity_y = init_velocity
                velocity_x = 0
    ...
    pygame.draw.rect(screen, (255,0,255), (rat_x, rat_y, size+1, size+1))
    # pygame.draw.rect(screen, (255,0,255), (velocity_x, velocity_y, size+1, size+1))
    # Fill the background with white
    # Draw a solid blue circle in the center
    # Flip the display
    j+=1
